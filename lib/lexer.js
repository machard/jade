...
Lexer.prototype = {
  ...
  /**
  * Tags modifiers : (attributesOf|remove|contentOf|prependTo|appendTo|insertNextTo|insertBefore) tagSelector
  */

  tagModifier: function() {
    var captures;
    if (captures = /^(attributesOf|remove|contentOf|prependTo|appendTo|insertNextTo|insertBefore) +/.exec(this.input)) {
      this.consume(captures[0].length);
      var mode = captures[1]
        , tok = this.tok('tagModifier');
      tok.mode = mode;
      return tok;
    }
  },
